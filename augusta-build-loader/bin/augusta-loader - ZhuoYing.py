...
# Joey Zhang <zhangzzz@cn.ibm.com>
# Liang Zhou <shzhoul@cn.ibm.com>
#
...
TEVEL_MIRROR_URL = 'http://9.115.235.138/1.0/official/'
...
def cmd_install_virtapp(users, stream, build=None, adhoc=False, develop_mode=False):
    ...
    try:
        ...
        for user in profile_names:
            ...
            property_path = os.path.join(img_mount, 'opt/ibm/augusta/data/runtime.properties')
            ...
            if os.path.exists(profile_dir):
                ...
                route_path = os.path.join(profile_dir, 'route-eth0')
                
                if os.path.isfile(ifcfg_path):
                    shutil.copy(ifcfg_path, local_config_dir)
                
                if os.path.isfile(route_path):
                    shutil.copy(route_path, local_config_dir)

                if develop_mode:
                    print "Develop mode: changing runtime.properties"
                    subprocess.call("echo 'com.ibm.sysx.xhmc.discovery.management.skipLDAPConfig=true' >> %s" % property_path, shell=True)
            ...
            sys.stdout.flush()
            ret = subprocess.call("virt-install --import -n %(vm_name)s -r %(vm_mem)d --vcpus=%(vm_cpu)d -v --graphics vnc,listen=0.0.0.0 --disk path=%(vm_disk)s,format=qcow2 --network bridge=%(vm_bridge)s --network bridge=%(vm_bridge)s --noautoconsole --noreboot >/dev/null 2>&1" % 
